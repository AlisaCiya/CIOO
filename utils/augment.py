...
class RandomPerspective:
    ...
    def affine_transform(self, img, border):
        ...
        _translate = self.args.translate
    ...
    def __call__(self, labels):
        ...
        n = len(objs.bboxes)
        if n == 0:
            # No boxes: keep image after transform and return empty instances/cls in labels dict
            labels["instances"] = Instances(np.zeros((0, 4), dtype=np.float32),
                                            objs.segments, None,
        ...
        xy = (xy[:, :2] / xy[:, 2:3] if self.args.psp else xy[:,
                                                                :2])
...
def transforms(dataset, args):
    mosaic = Mosaic(args, dataset)

    affine = RandomPerspective(args, pre_transform=LetterBox(
    ...
    return Compose([pre_transform,
                    ...
                    Albumentations(), RandomHSV(args),
                    RandomFlip(direction="vertical", p=args.flip_ud),
                    RandomFlip(direction="horizontal", p=args.flip_lr)])
